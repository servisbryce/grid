/*

    We aim to provide a facility for the program to receive tasks, execute them, and return the results to the user.

*/

#include "../../include/controller.h"
#include <openssl/err.h>
#include <unistd.h>

/* We aim to provide a facility to handle any incoming connections that we may have. */
void *controller_network_task(void *controller_network_task_vargs_p) {

    /* Cast our pointer into the appropriate type and structure. */
    controller_network_task_vargs_t *controller_network_task_vargs = (controller_network_task_vargs_t *) controller_network_task_vargs_p;

    /* Ensure that the arguments the thread was provide actually exists. */
    if (!controller_network_task_vargs) {

	return (void *) -1;

    }

    /* If any heap allocated structures exist, we must destroy them or we'll have a major memory leak! */
    if (controller_network_task_vargs->client_sockaddr) {

	free(controller_network_task_vargs->client_sockaddr);

    }

    if (controller_network_task_vargs->ssl) {

	SSL_shutdown(controller_network_task_vargs->ssl);
	SSL_free(controller_network_task_vargs->ssl);

    }

    /* Finally, close the connection and free the structure. Then, return and let the thread be freed. */
    close(controller_network_task_vargs->client_sockfd);
    free(controller_network_task_vargs);
    return (void *) 0;

}
